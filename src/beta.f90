...
!> Kemna (2000): ERT surveys are generally conducted from both the earth`s surface and boreholes, defining a vertical 2D plane. The corresponding model geometry is characterized by homogeneous Neumann boundary conditions, i.e. \f$ \beta =0 \f$, at the surface (no current flow in the normal direction) and mixed boundary conditions, i.e. \f$ \beta \neq 0 \f$, at those boundaries confining the considered region within the half-space. In the following, these two types are referred to as <I>surface</I> and <I>half-space</I> boundaries, respectively.
!> 
!> Due to their much easier implementation, most early resistivity modeling algorithms impose homogeneous Dirichlet conditions in the half-space boundaries, i.e. \f$ \tilde \phi = 0 \f$ (corresponding to \f$ \beta =\inf \f$). This consequently requires the usage of large modeling grids, extending far to sides and depth from the actual region of interest, in order to keep related modeling errors at an acceptable level. However, here an approach first suggested by Dey and Morrison (1979) is used instead, where an adequate, finite value for  on the half-space boundaries is determined via the asymptotic behavior of the potential for a homogeneous half-space. Such a mixed (or asymptotic) boundary condition enables the usage of comparatively small grids and, thus, reduces the overall computational requirements for the modeling.
!> 
