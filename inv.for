...
cdiff-        call kont1(delem,delectr,dstrom,drandb)
cdiff+<
...
ctst            if (dabs(1d0-nrmsd/nrmsdm).le.mqrms) errnr2=80
...
c SENSITIVITAETEN berechnen
            ...
            if (lsetup) then
cakc Ggf. Summe der Sensitivitaeten aller Messungen ausgeben
c                if (lsens) then
c                    if (ldc) then
...
c Kontrollausgabe
     ...
     1                    ' : Calculating 1st regularization parameter'
                        call blam0()
                        lam = lammax
cak Model EGS2003, ERT2003                        call blam0()
cak Model EGS2003, ERT2003                        lam = lammax
cak                        lam = 1d4
                    else
                        dlalt = dlam
                        if (ldlami) then
...
ctst                        if (dlam.gt.1d0) lfstep=.true.
cak Model EGS2003
